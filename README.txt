...
However, often times I will find myself having to pull that SQL+parameters out of SQL Profiler and paste it into SQL Server Studio in order to analyze, tweak, debug, etc. However, SQL Profiler's output looks like:
...
Which is fine, and *excutes* in SQL Studio, but if there's a syntax error, you can't really pinpoint exactly where in the query the error is, and it's harder to manipulate the parameters because you have to do it in two spots, and it's just time-consuming if you're doing this a lot (which I was).
...
* If the query uses WITH, then I need to add semicolons after the parameters.